class Api::ApiController < ApplicationController
  ...
  def authenticate_user
    ...
    if authentication[:type] == 'jwt' && authentication[:claims]['pw_hash']
      ...
      if ActiveSupport::SecurityUtils.secure_compare(pw_hash, encrypted_password_digest) == false
        render_invalid_auth
        return
      end
    end
  ...
  def validate_token(token)
    ...
    { type: 'session_token', user_uuid: session.user_uuid, session: session } unless session.nil?
  end
end
